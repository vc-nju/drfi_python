...
class Utils():
    ...
    def get_edges(self):
        ...
        '''
        shape = (self.rmat[0], self.rmat[1], 8, )
        edge_mat = np.zeros(shape, dtype=np.int32)
        edge_mat[:-1, :, 0] += (self.rmat[1:, :] - self.rmat[:-1, :])
        edge_mat[1:, :, 1] += (self.rmat[:-1, :] - self.rmat[1:, :])
        edge_mat[:, :-1, 2] += (self.rmat[:, 1:] - self.rmat[:, :-1])
        edge_mat[:, 1:, 3] += (self.rmat[:, :-1] - self.rmat[:, 1:])
        edge_mat[:-1, :-1, 4] += (self.rmat[1:, 1:] - self.rmat[:-1, :-1])
        edge_mat[:-1, 1:, 5] += (self.rmat[1:, :-1] - self.rmat[:-1, 1:])
        edge_mat[1:, :-1, 6] += (self.rmat[:-1, 1:] - self.rmat[1:, :-1])
        edge_mat[1:, 1:, 7] += (self.rmat[:-1, :-1] - self.rmat[1:, 1:])
        ...
        shape = (self.rmat[0], self.rmat[1], 2, 8)
        y_x = np.zeros(shape)
        for y in range(shape[0]):
            for x in range(shape[1]):
                y_x[y,x,0,:] = generate_y_list(y)
                y_x[y,x,1,:] = generate_x_list(x)
        edge_nums = np.zeros([len(self.rlist), 1])
        edge_neigh = []
        edge_point = []
        ...
        for region in self.rlist:
            ...
            for y, x in zip(region[0], region[1]):
                for edge_direct in edge_mat.shape[2]:
                    if edge_direct[y, x, edge_direct] != 0:
                        y_ = y_x[y, x, 0, edge_direct]
                        x_ = y_x[y, x, 1, edge_direct]
                        num += 1
                        points = append_not_exist((y_, x_, ), points)
                        neighs = append_not_exist(self.rmat[y_, x_], neighs)
