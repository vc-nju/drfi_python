...
class Features():
    ...
    def get_background_features(self):
        ...
        dot = functools.partial(utils.dot, bkg=True)
        for i in range(9):
            bkg_features[:, i] = dot(utils.color_avg[:, i])[:-1]
        bkg_features[:, 9] = dot(self.rgb, hist=True)[:-1]
        bkg_features[:, 10] = dot(utils.hsv, hist=True)[:-1]
        bkg_features[:, 11] = dot(utils.lab, hist=True)[:-1]
        for i in range(15):
            bkg_features[:, i+12] = dot(utils.tex_avg[:, i])[:-1]
        bkg_features[:, 27] = dot(utils.tex, hist=True)[:-1]
        bkg_features[:, 11] = dot(utils.lbp, hist=True)[:-1]
        return bkg_features
